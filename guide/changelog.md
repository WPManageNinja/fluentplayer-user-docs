...
- **Dynamic shortcode source** — override a media item's saved source right from the shortcode using `src`, `meta_key`, and `poster` attributes (for example, `[fluentplayer id="12" meta_key="video_url"]` to read the URL from a post's custom field). See [Embed with Shortcode](/guide/display-embed/shortcode).
